import { Box, Button, Container, HStack, RadioGroup, Radio, VStack, Text, Image, Stat, StatLabel, StatNumber, StatHelpText, StatArrow, Badge, Progress } from "@chakra-ui/react";
...
const CoinDetails = () => {
  ...
  if (error) return <Error message={"Error while loading Coin"} />;
        ...
        loading?<Loader />:(
          <>
            ...
            <HStack p={'4'} wrap={"wrap"}>
              {
                btns.map((i)=>(
                  <Button key={i}m={'2'} onClick={()=>switchChartStats(i)}> {i} </Button>
                ))
              }
            </HStack>
